    ...
    def getAction(self, state):
        ...
        for i in range(len(optimalActions)):
            ...
            if studentAction[1] == int(optimalActions[i][1]):
                ...
                compTuples.append((studentAction[1], int(optimalActions[i][1])))
        if not curRightStatesExplored:# and self.wrongStatesExplored < 0:
            self.wrongStatesExplored = 1
            print("error! Wrong states explored! No match in ", compTuples)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, 6*len(thresholds)))
            ...
            if minimum != None:
                ...
                for idx, threshold in enumerate(thresholds):
                    self.addMessage("    >= %s:  %s points" % (threshold, 6*(idx+1)))
